...
class BwListener(val mirror: Mirror, var replayNum: Int, val dbName: String, val mapOnly: Boolean)
  ...
  override def onEnd(b: Boolean): Unit = {
    ...
    log.info("Replay Ended, took {}", time)
  }
...
object BwListener {
  val unitTypes = Seq[bwapi.UnitType](
  ...
  val techTypes =
    Seq[bwapi.TechType](bwapi.TechType.Stim_Packs,
      ...
      bwapi.TechType.Nuclear_Strike)

  val upgradeTypes =
    Seq[bwapi.UpgradeType](bwapi.UpgradeType.Terran_Infantry_Armor,
